...
function createCrackTexture(renderer) {
  ...
  const drawCrack = (points, width, alpha) => {
    ...
    ctx.beginPath();
    points.forEach((p, i) => (i === 0 ? ctx.moveTo(p.x + 3, p.y - 2) : ctx.lineTo(p.x + 3, p.y - 2)));
    ctx.stroke();
    ...
    ctx.beginPath();
    points.forEach((p, i) => (i === 0 ? ctx.moveTo(p.x - 2, p.y + 1) : ctx.lineTo(p.x - 2, p.y + 1)));
    ctx.stroke();
  };
  ...
  seeds.forEach((seed, si) => {
    ...
    for (let s = 0; s <= segmentCount; s++) {
      ...
      const angle  = seed.angle + Math.sin(s * 2.45 + si * 0.8) * 0.13 + Math.cos(s * 1.25 + si) * 0.06;
      const radius = size * seed.length * t * (0.45 + Math.sin(t * Math.PI) * 0.12);
    ...
    for (let bi = 0; bi < branchCount; bi++) {
      ...
      const bAngle    = seed.angle + (bi % 2 === 0 ? 1 : -1) * (0.62 + Math.random() * 0.42);
      const bLen      = size * seed.length * (0.12 + Math.random() * 0.1);
      const bPoints   = [root];
      for (let s = 1; s <= 4; s++) {
        const t     = s / 4;
        const angle = bAngle + Math.sin(s * 1.9 + si) * 0.12;
        bPoints.push({ x: root.x + Math.cos(angle) * bLen * t, y: root.y + Math.sin(angle) * bLen * t });
      }
...
export default function HeroScene({
  ...
  useEffect(() => {
    ...
    const revealLightMaterial = new THREE.MeshBasicMaterial({ color: '#ffe2a0', transparent: true, opacity: 0, depthWrite: false, blending: THREE.AdditiveBlending, toneMapped: false, side: THREE.DoubleSide });

    // Barst-overlay die over de tegel getekend wordt tijdens de ontgrendeling.
    const crackOverlayMaterial = new THREE.MeshBasicMaterial({ color: '#ffffff', map: crackTexture, transparent: true, opacity: 0, depthWrite: false, toneMapped: false });
    ...
    const crackMaterial = new THREE.MeshBasicMaterial({ color: '#173f8c', transparent: true, opacity: 0, depthWrite: false, toneMapped: false });
    const crackLines    = [];

    const crackSeeds = [
      { angle: -2.75, length: 0.54 }, { angle: -2.18, length: 0.38 }, { angle: -1.62, length: 0.55 },
      { angle: -1.12, length: 0.42 }, { angle: -0.56, length: 0.58 }, { angle: -0.08, length: 0.46 },
      { angle:  0.48, length: 0.56 }, { angle:  1.02, length: 0.42 }, { angle:  1.54, length: 0.54 },
      { angle:  2.1,  length: 0.46 }, { angle:  2.64, length: 0.6  },
    ];
    ...
    const createCrackTube = (points, radius, opacityScale = 1) => {
      const mesh       = new THREE.Mesh(new THREE.TubeGeometry(new THREE.CatmullRomCurve3(points), 24, radius, 7, false), crackMaterial.clone());
      mesh.renderOrder = 9;
    ...
    crackSeeds.forEach((seed, si) => {
      ...
      for (let s = 0; s <= segCount; s++) {
        const t     = s / segCount;
        const kink  = Math.sin(s * 2.7 + si) * 0.08 + Math.cos(s * 1.3 + si * 0.7) * 0.035;
        const angle = seed.angle + kink;
        const r     = seed.length * t * (0.9 + Math.sin(t * Math.PI) * 0.08);
        pts.push(new THREE.Vector3(Math.cos(angle) * r, Math.sin(angle) * r, TILE_DEPTH / 2 + 0.018));
      }
      ...
      if (si % 2 === 0) {
        ...
        const bAngle = seed.angle + (si % 4 < 2 ? 1 : -1) * (0.55 + Math.random() * 0.38);
        const bLen   = seed.length * (0.24 + Math.random() * 0.18);
        const bPts   = [root];
        for (let s = 1; s <= 4; s++) {
          const t    = s / 4;
          const kink = Math.sin(s * 1.8 + si) * 0.05;
          bPts.push(new THREE.Vector3(root.x + Math.cos(bAngle + kink) * bLen * t, root.y + Math.sin(bAngle + kink) * bLen * t, TILE_DEPTH / 2 + 0.019));
        }
    ...
    for (let row = 0; row < shardRows; row++) {
      for (let col = 0; col < shardCols; col++) {
        ...
        const triangles = fwd
          ? [[{ x: x0, y: y0 }, { x: x1, y: y0 + Math.random() * 0.035 }, { x: cx, y: cy }],
             [{ x: x1, y: y1 }, { x: x0 + Math.random() * 0.035, y: y1 }, { x: cx, y: cy }]]
          : [[{ x: x0, y: y1 }, { x: x0, y: y0 - Math.random() * 0.035 }, { x: cx, y: cy }],
             [{ x: x1, y: y0 }, { x: x1, y: y1 + Math.random() * 0.035 }, { x: cx, y: cy }]];

        triangles.forEach((pts, ti) => {
          const shard     = new THREE.Mesh(createShardGeometry(pts), lockedMaterial.clone());
    ...
    for (let i = 0; i < 18; i++) {
      ...
      rGeo.setAttribute('position', new THREE.BufferAttribute(new Float32Array([
        0, 0, 0, -rW, rLen, 0, rW, rLen * (0.82 + Math.random() * 0.28), 0,
      ]), 3));
    ...
    const animate = () => {
      ...
      if (introActive && !startIntroRef.current && floatingTile.visible && wallEntranceActive) {
        ...
        floatingTile.position.set(targetX, THREE.MathUtils.lerp(flyFromY, targetY, ease), 0.07);
        floatingTile.rotation.set((1 - ease) * -0.18, (1 - ease) * 0.12, (1 - ease) * 0.04);
        floatingTile.scale.set(tileWidth, tileHeight, 1);
        floatingMaterials.forEach((m) => { m.opacity = 1; });
      ...
      if (introActive && startIntroRef.current) {
        ...
        revealRays.forEach((r) => { r.visible = false; r.material.opacity = 0; });
        ...
        if (crackOverlay) {
          ...
          crackOverlay.material.opacity = ot * Math.max(0, 1 - smootherStep(THREE.MathUtils.clamp((introT - 0.74) / 0.14, 0, 1)));
        ...
        crackLines.forEach((line, i) => {
          const lineT  = THREE.MathUtils.clamp((crackT - i * 0.022) / 0.72, 0, 1);
          const lineE  = smootherStep(lineT);
          line.material.opacity = (0.06 + Math.sin(lineT * Math.PI) * 0.12 + lineE * 0.2) * line.userData.opacityScale
            * Math.max(0, 1 - smootherStep(THREE.MathUtils.clamp((introT - 0.72) / 0.16, 0, 1)));
        ...
        revealShards.forEach((shard, i) => {
          const localDelay = (i % 9) * 0.009 + Math.hypot(shard.userData.centroidX, shard.userData.centroidY) * 0.04;
          ...
          shard.rotation.set(shard.userData.rotX * blast + flutter, shard.userData.rotY * blast, shard.userData.rotZ * blast + preCrack);
        ...
        floatingTile.position.set(
          THREE.MathUtils.lerp(targetX, startX, approachEase) + Math.sin(elapsed * 68) * 0.035 * shakePulse,
          THREE.MathUtils.lerp(targetY, startY, approachEase) + Math.sin(approachT * Math.PI) * tileHeight * 0.45 + Math.cos(elapsed * 59) * 0.025 * shakePulse,
      ...
      if (!insertionStarted && !inserted && !extracting) overlayOpacity = THREE.MathUtils.damp(overlayOpacity, FOCUS_OVERLAY_OPACITY, 5, delta);
      ...
      focusOverlay.visible    = overlayOpacity > 0.003 && externalOverlayRef.current && wallEntranceComplete;
      ...
      tiles.forEach((mesh, i) => {
        ...
        let ePX = mesh.userData.baseX, ePY = mesh.userData.baseY, eRX = 0, eRY = 0, eRZ = 0;
        ...
        mesh.position.set(ePX + radialX * wave * damp * 0.24 + tremor * 0.075, ePY - radialY * wave * damp * 0.24 + tremor * 0.012, wave * damp * 0.14 + hover * 0.04);
        mesh.scale.set(tileWidth, tileHeight, 1);
    ...
    return () => {
      ...
      [particleMaterial, shockwaveMaterial, idleGlowMaterial, overlayMaterial].forEach((m) => m.dispose());
      ...
      crackLines.forEach((l)   => { l.geometry.dispose(); l.material.dispose(); });
